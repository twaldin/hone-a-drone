...
Without this, hone spawns each claude-code mutator call in a fresh tmpdir,
which means the mutator doesn't see /Users/twaldin/dev/hone-a-drone/CLAUDE.md
(claude-code walks UP from cwd; from /tmp/hone-mutator-XXX/ it never reaches
the project dir). Pinning workdir = project root lets the mutator read
CLAUDE.md with the planner-specific guidance.
...
def main():
    ...
    mutator = HarnessMutator(
        ...
        model="sonnet",
        workdir=PROJECT_ROOT,       # so claude-code sees the project CLAUDE.md
        timeout_seconds=600,
    )
