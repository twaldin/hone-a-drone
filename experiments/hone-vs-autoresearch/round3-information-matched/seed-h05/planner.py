...
EXIT_DIST: float = 0.62        # m after gate center, along exit direction
LIFTOFF_FRAC: float = 0.55     # fraction of height to first gate used for liftoff wpt
MIN_SEGMENT_TIME: float = 0.49  # s minimum time per waypoint segment
OBSTACLE_RADIUS: float = 0.2  # m safety bubble around each obstacle axis (xy)
OBSTACLE_AVOID_OFFSET: float = 0.35  # m lateral offset when nudging gate staging waypoints
SEGMENT_DETOUR_RADIUS: float = 0.18  # m; only detour clear straight segments with tight obstacle clips
SEGMENT_DETOUR_OFFSET: float = 0.28  # m; gentler mid-segment offset to reduce out-of-bounds arcs
STACKED_SWITCH_RADIUS: float = 0.22  # m; switch stacked-gate staging before hover error stalls
VELOCITY_FF_GAIN: float = 0.25  # small spline velocity feed-forward to reduce tracking lag
ACCEL_FF_GAIN: float = 0.03  # tiny curvature feed-forward for crisper gate alignment
...
class Planner:
    ...
    def compute_target(self, obs: dict, info: dict | None, t: float) -> np.ndarray:
        ...
        vel = VELOCITY_FF_GAIN * self._spline_vel(t_c)
        acc = ACCEL_FF_GAIN * self._spline_acc(t_c)
        ...
        if self._stacked_track and 0 <= gate_idx < self._n_gates:
            ...
            if self._gate_phase == 0 and float(np.linalg.norm(current_pos - approach)) < STACKED_SWITCH_RADIUS:
                self._gate_phase = 1
        ...
        return make_state_command(pos, vel=vel, acc=acc)
...
def _build_waypoints(
    ...
    for i, (pos, quat) in enumerate(zip(gates_pos, gates_quat)):
        ...
        if obstacles_pos is not None and len(obstacles_pos) > 0:
            ...
            detour = _segment_obstacle_detour(wpts[-1], approach_wpt, obstacles_pos, normal)
...
def _segment_obstacle_detour(
    start: np.ndarray,
    end: np.ndarray,
    obstacles_pos: np.ndarray,
    normal: np.ndarray,
) -> np.ndarray | None:
    """Add a small lateral detour when a straight segment clips an obstacle bubble."""
    seg = end[:2] - start[:2]
    ...
    perp = perp / perp_n if perp_n > 1e-6 else np.array([0.0, 1.0, 0.0])
    best: np.ndarray | None = None
    best_margin = SEGMENT_DETOUR_RADIUS
    for o in obstacles_pos:
        u = float(np.clip(np.dot(o[:2] - start[:2], seg) / seg_len2, 0.0, 1.0))
        if u < 0.08 or u > 0.92:
            continue
        closest = start + u * (end - start)
        miss = closest[:2] - o[:2]
        margin = float(np.linalg.norm(miss))
        if margin < best_margin:
            side = float(np.dot(perp[:2], miss))
            if abs(side) < 0.03:
                # Ambiguous near-centerline clips: bend toward the course center
                # rather than arbitrarily outward, reducing boundary excursions.
                side = -float(np.dot(perp[:2], closest[:2]))
            sign = 1.0 if side >= 0.0 else -1.0
            candidate = closest + sign * SEGMENT_DETOUR_OFFSET * perp
            # Avoid letting obstacle detours expand the course envelope much beyond
            # the segment endpoints; out-of-bounds is usually worse than a small clip.
            endpoint_r = max(float(np.linalg.norm(start[:2])), float(np.linalg.norm(end[:2])))
            if float(np.linalg.norm(candidate[:2])) > endpoint_r + 0.25:
                candidate = closest + sign * (0.65 * SEGMENT_DETOUR_OFFSET) * perp
            best = candidate
            best_margin = margin
    return best
